...
def exchange(str):
    ...
    if len(str) == 1:
        ...
        exchange_str = end + str[1:-1] + start
...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    range = [x for x in alist if (x > max or x < min)]
    return range
...
def word_count_in_set(words):
    ...
    *** Solution should not be more than 3 lines of code (can be less)
    ...
    """
    newSetWords = set(words)
    length = len(newSetWords)
    return length
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """
    word_count = {x : strings.count(x) for x in strings}
    return word_count
...
def friend_list(friend_dictionary):
    ...
    for x in friend_dictionary:
        if (friend_list.__contains__(x) == False):
            friend_list[x] = [friend_dictionary[x]]
        if(friend_list.__contains__(friend_dictionary[x]) == False):
            friend_list[friend_dictionary[x]] = [x]
        if(friend_list.__contains__(friend_dictionary[x])):
            old_friend = friend_list[friend_dictionary[x]]
            if x not in old_friend:
                friend_list[friend_dictionary[x]].append(x)
 ...
 *** Important: your code must use comprehensions and should not be more than
 ...
 """
 return len([s for s in string if s == ' '])


# Test functions
assert count_spaces("the cow jumped over the moon") == 5
print("correct")
